import { Alert, Button, Divider, message, Popover, Space, Tabs } from 'antd';
...
import type { LoginTypeName, LoginTypeValue } from '@/services/fuck-web/enums';
import { LoginType } from '@/services/fuck-web/enums';
import { sendLoginCaptcha, userLogin as login } from '@/services/fuck-web/user';
...
const Login: React.FC = ({}) => {
  ...
  useEffect(() => {
    if (globalConfig) {
      ...
      if (globalConfig.defaultLoginType === LoginType.oauth2) {
        ...
        handleSetLoginType(LoginType.oauth2);
      } else {
        setAllowLoginTypes(
          globalConfig.loginType
            .map((item) => item.type)
  ...
  console.log(otherLoginTypes, allowLoginTypes, hiddenNormal, loginType);
  useEffect(() => {
    setTabsItem(
      enumToOptions(
        LoginType,
          ...
          oauthLoginTypes.length > 0 ? (
            <div className="ant-pro-form-login-page-other">
              ...
              >
                <Space
                  align="center"
                  className={styles.oauthLoginTypes}
                  size={24}
                  key="loginMethod"
                >
              ...
              <div
                className={styles.loginByOtherBtn}
                hidden={
                  (globalConfig?.loginType.filter((item) => item.type !== LoginType.oauth2) ?? [])
                    .length == 0 || globalConfig?.defaultLoginType !== LoginType.oauth2
                }
              >
                <Button
