...
type ListRequest<T extends ListItem> = (params: {
  pageSize?: number;
  current?: number;
  keywords?: string;
...
const List = <T extends ListItem>({
  ...
  return (
    <ProList<T>
      ...
      metas={{
        title: { render: (_, entry) => entry.displayName ?? entry.name },
        subTitle: {
          render: (_, entry) => {
            if (entry.isDisable) {
        ...
        content: {
          render: (_, entry) => {
            return (
              <Typography.Paragraph ellipsis={{ rows: 2, tooltip: entry.description }}>
                {entry.description ?? ''}
